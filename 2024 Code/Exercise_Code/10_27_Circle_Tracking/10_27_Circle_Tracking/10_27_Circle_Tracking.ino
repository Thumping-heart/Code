...
//存传感器数据
bool Sensor_Front_Data[9] = {0};//前方传感器  0
bool Sensor_Rear_Data[9] = {0};//后方传感器   1
bool Sensor_Left_Data[9] = {0};//左侧传感器   2
bool Sensor_Right_Data[9] = {0};//右侧传感器  3
...
void loop() {
  ...
  {
    Straight(50);
    if(Sensor_Right_Data[8] == 1)
    {
      Step = 3;
    }
  }
  else if(Step == 3)//逆时针旋转
  {
    MA_SPD(50);
    MB_SPD(50);
    MC_SPD(-50);
  ...
  else if(Step == 5)//逆时针转
  {
    ...
    MC_SPD(-50);
    MD_SPD(-50);
    if(Sensor_Front_Data[1] == 1 && millis() - Run_Time > 1000)
    {
      Step = 6;
    }
  }
