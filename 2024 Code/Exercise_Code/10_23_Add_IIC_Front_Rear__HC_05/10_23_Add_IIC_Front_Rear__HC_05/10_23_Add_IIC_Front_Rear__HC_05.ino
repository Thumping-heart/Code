...
#include <SoftwareSerial.h>//蓝牙库文件

SoftwareSerial BT(10,9); //PIN10接蓝牙的TXD   PIN9接蓝牙的RXD
char Blue_Tooth_Data = 'A';//定义一个变量存蓝牙有关的数据 初值为A
...
//  Print_Sensor();
  ...
  {
    Blue_Tooth_Data = Serial.read();              //把写入的数据给到自定义变量  X
    BT.print(Blue_Tooth_Data);                    //把数据给蓝牙
  }

  if (BT.available())               //检测：【蓝牙】如果数据写入，则执行。
  {
    Blue_Tooth_Data = BT.read();                  //把检测到的数据给到自定义变量 X
    Serial.print(Blue_Tooth_Data);                //把从蓝牙得到的数据显示到串口监视器
  }
  
  if(Blue_Tooth_Data == 'G')//前
  {
    Straight(60);
    Serial.println("Straight");
  }
  else if(Blue_Tooth_Data == 'K')//后
  {
    Straight(-60);
    Serial.println("Rear");
  }
  else if(Blue_Tooth_Data == 'H')//左
  {
    Left_Shift(60);
    Serial.println("Left");
  }
  else if(Blue_Tooth_Data == 'J')//右
  {
    Left_Shift(-60);
